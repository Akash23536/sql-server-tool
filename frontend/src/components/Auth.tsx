  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-slate-900 text-white relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md p-8 bg-slate-800/60 backdrop-blur-xl rounded-2xl border border-slate-700 shadow-2xl">
        ...
        {dbStatus === 'disconnected' && !error && (
          <div className="mb-6 p-4 rounded-lg bg-amber-500/10 border border-amber-500/50 text-amber-200 text-xs leading-relaxed">
            <div className="font-bold uppercase tracking-widest text-amber-500 mb-1">Connection Issue</div>
            The system cannot reach MongoDB Atlas. Please ensure your IP is whitelisted or use <strong>Guest Access</strong> below for testing.
          </div>
        )}
        ...
        <div className="mt-6">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  if (response.ok) onLogin(data.token, data.username);
              ...
              className="flex items-center justify-center py-2.5 px-4 border border-slate-700 rounded-xl hover:bg-slate-700/50 transition-all group"
              title="Guest Access"
            >
              ...
              <span className="ml-2 text-xs font-bold text-slate-300">Guest</span>
            </button>

            <button 
              onClick={() => alert("Google Login would be configured here with a Client ID.")}
              className="flex items-center justify-center py-2.5 px-4 border border-slate-700 rounded-xl hover:bg-slate-700/50 transition-all group"
            >
            ...
            <button 
              onClick={() => alert("GitHub Login would be configured here with a Client ID.")}
              className="flex items-center justify-center py-2.5 px-4 border border-slate-700 rounded-xl hover:bg-slate-700/50 transition-all group"
            >
      ...
      <a 
        ...
        className="absolute bottom-8 right-8 z-20 flex items-center gap-3 group transition-all hover:scale-105 active:scale-95 bg-slate-900/20 backdrop-blur-sm px-4 py-2 rounded-full border border-white/5 hover:border-white/10 shadow-xl"
      >
